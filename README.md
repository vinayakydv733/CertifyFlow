...
6. Run the frontend with `pnpm dev` and the API with `pnpm server` during development. For production, run `pnpm build` and start the API with `pnpm start`; serve `dist/` from your HTTPS host and proxy `/api` to the API process.
